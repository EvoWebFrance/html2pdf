...
class Html2Pdf
{
		 ...
		 * @param string $type Debug type : 'normal' or 'ajax'
		 *
		 * @return Html2Pdf $this
		 */
		public function setModeDebug(DebugInterface $debugObject = null , $type='normal')
		{
				if (is_null($debugObject)) {
						$this->debug = new Debug($type);
				} else {
		...
		public function getDebugContent() {
			if (!is_null($this->debug))
				return $this->debug->debugContent;
			else
				return "";
		}
		...
		public function Output($name = '', $dest = false)
		{
				...
				if (!is_null($this->debug)) {
						$this->debug->stop();
						$this->pdf->Close();
						if ($this->debug->debugType == 'normal') return '';
				}
